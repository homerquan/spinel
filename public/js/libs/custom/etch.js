// A customized etchjs for reflen
// change save function to adopte 2 step model save
// by homer quan (contact@homerquan.com)
(function() {
  'use strict';
  ...
  etch.config = {
    ...
    // data-field in dom attribute to bind model attribute 
  ...
  views.Editor = Backbone.View.extend({
    ...
    changeButtons: function() {
      ...
      _.each(this.model.get('buttons'), function(button) {
        var $buttonEl = $('<a href="#" data-bypass="true" class="etch-editor-button etch-' + button + '" title="' + button.replace('-', ' ') + '"><span></span></a>');
    ...
    save: function(e) {
      ...
      var editableElem = this.model.get('editable');
      var dataField = this.getField();
      // update model attribute
      editableModel.set(dataField, this.getUpdateValue());
      editableModel.trigger('updateField', dataField);
    },
    getField: function() {
      return this.model.get('editable')[0].getAttribute(etch.config.dataField);
    },
    getUpdateValue: function() {
      return this.model.get('editable')[0].innerHTML;
    }
  });
  ...
  _.extend(etch, {
    ...
    editableInit: function(e) {
      ...
      editorModel.set({
        position: {
          x: $(e.currentTarget).position().left,
          y: $(e.currentTarget).position().top + 20
        }
      });
    }
  });
